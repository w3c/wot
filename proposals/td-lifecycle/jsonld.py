...
class JsonLdProcessor(object):
    ...
    def compact(self, input_, ctx, options):
        ...
        if options['skipExpansion']:
            ...
            #Lee print ("expand",json.dumps(expanded) ) #Lee
        ...
        #Lee print ("active_ctx",json.dumps(active_ctx) ) #Lee

        # do compaction
        compacted = self._compact(active_ctx, None, expanded, options)

        #Lee print ("do compaction:",json.dumps(compacted) ) #Lee
        ...
        if (options['compactArrays'] and not options['graph'] and
            ...
            #Lee print ("not graph compaction:",json.dumps(compacted) ) #Lee
        # always use an array if graph options is on
        elif options['graph']:
            compacted = JsonLdProcessor.arrayify(compacted)
            #Lee print ("is graph compaction:",json.dumps(compacted) ) #Lee
        ...
        #Lee print ("build output context:",json.dumps(ctx) ) #Lee
        ...
        #Lee print ("remove empty contexts:",json.dumps(ctx) ) #Lee
        ...
        #Lee print ("remove array if only one context:",json.dumps(ctx) ) #Lee
    ...
    def _expand(
        ...
        if _is_array(element):
            ...
            for e in element:
                ...
                for key, value in sorted(e.items()):
                    if key == '@include' and not _is_string(value) and not _is_object(value):
                        raise JsonLdError(
                                    'Invalid JSON-LD syntax; "@id" value must be a '
                                    'string or an object.', 'jsonld.SyntaxError',
                                    {'value': value}, code='invalid @id value')
                    if key == '@include' and _is_string(value):
                        expanded_value = self._expand_iri(active_ctx, value, vocab=True)
                        ss = expanded_value.split('#')
                        incappdt = expand(ss[0])
                        for inc_elem in incappdt:
                            for inc_key, inc_value in inc_elem.items():
                                chk_flag = False
                                if '#' in inc_key:
                                   inc_key_ss = inc_key.split('#')
                                   if inc_key_ss[1]==ss[1] and _is_array(inc_value):
                                      chk_flag = True
                                else:
                                   if inc_key==ss[1] and _is_array(inc_value):
                                      chk_flag = True
                                if chk_flag==True:
                                   for obj in inc_value:
                                      rval.append(obj)
                                   #return rval
                                
                    elif key == '@include' and _is_object(value):
                        for incobj_key, incobj_value in sorted(value.items()):
                           #print("incobjKey=", incobj_key)
                           #print("incobjValue=", incobj_value)
                           if incobj_key == '@id' and _is_string(incobj_value):
                              expanded_value = self._expand_iri(active_ctx, incobj_value, vocab=True)
                              ss = expanded_value.split('#')
                              #print( "incobj_value:" , incobj_value )
                              #print( "SS0:" , ss[0] )
                              #print( "SS1:" , ss[1] )
                              incappdt = expand(ss[0])
                              #print( "IncList:", json.dumps(incappdt))
                              for inc_elem in incappdt:
                                  for inc_key, inc_value in inc_elem.items():
                                     chk_flag = False
                                     if '#' in inc_key:
                                     ...
                                     if chk_flag==True:
                                        for obj in inc_value:
                                           #print("Append:", json.dumps(obj))
                                           rval.append(obj)
                           if incobj_key == '@remove' and _is_array(incobj_value):
                              #####################
                              # {'xxxxx:name':[{"@value":"Text"}]
                              #####################
                              for rvalobj in rval:
                                  if  _is_object(rvalobj):
                                     #print("rvalobj:", json.dumps(rvalobj))
                                     for rval_key, rval_value in rvalobj.items():
                                        if 'name' in rval_key:
                                           #print("rvalobj name:", json.dumps(rval_value))
                                           for rval_value_lst in rval_value:
                                               #print("rval_value_lst:", json.dumps(rval_value_lst))
                                               for k,v in rval_value_lst.items():
                                                 #print("k:", k)
                                                 #print("v:", v)
                                                 if k=='@value' and v in incobj_value:
                                                    #print("Remove:", json.dumps(rvalobj))
                                                    rval.remove(rvalobj)
                                                    break
                                               break
                                      
                           if incobj_key == '@substitute' and _is_object(incobj_value):
                              sname = ''
                              for incobjitem_key, incobjitem_value in incobj_value.items():
                                  #print("incobjitem_key:", incobjitem_key)
                                  #print("incobjitem_value:", incobjitem_value)
                                  if 'name' in incobjitem_key:
                                     sname = incobjitem_value
                                     #print("incsubstitute:", json.dumps(incobj_value))
                                     break
                              if len(sname)>0:
                                 #print("Sub Name:", sname)
                                 #####################
                                 # {'xxxxx:name':[{"@value":"Text"}]
                                 #####################
                                 for rvalobj in rval:
                                     if  _is_object(rvalobj):
                                        #print("rvalobj:", json.dumps(rvalobj))
                                        for rval_key, rval_value in rvalobj.items():
                                           #print("rvalobj_key:", rval_key)
                                           #print("rvalobj_value:", rval_value)
                                           if 'name' in rval_key:
                                              #print("rvalobj name:", json.dumps(rval_value))
                                              for rval_value_lst in rval_value:
                                                 #print("rval_value_lst:", json.dumps(rval_value_lst))
                                                 for k,v in rval_value_lst.items():
                                                    #print("k:", k)
                                                    #print("v:", v)
                                                    if k=='@value' and v==sname:
                                                       #print("Sub Remove:", json.dumps(rvalobj))
                                                       rval.remove(rvalobj)
                                                       e = self._expand(
                                                                 active_ctx, active_property, incobj_value, options, inside_list)
                                                       #print("Sub Append:", json.dumps(e))
                                                       rval.append(e)
                                                       break
                                                 break
                                              break
        ...
        for key, value in sorted(element.items()):
            ...
            #print ("Key=",key)
            #print ("value=",value)
            #print ("expanded_property=",expanded_property)
            #Lee
