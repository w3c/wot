...
See [https://github.com/w3c/wot/pull/1264](https://github.com/w3c/wot/pull/1264), OK to merge.
...
Cris: Do not introduce specific terms
... essentially "won't fix"
...
... impossible to say in the Scripting API that profile X is supported (mechanism is too open)
Cris: Profile is somewhat fuzzy
... no extra mechanism is applied when seeing the term "profile"
... the Scripting API does not support xxxAll terms like queryAll
Ege: Theoretically, it could be mapped to MQTT...
... queryAll would require relaxing dataSchema
Cris: There is no data schema—that's a problem
... we could do the same as we did for async actions
Daniel: We did that for a purpose... I would not do that
Ege: We can just provide examples on top of node-wot, but not as part of node-wot
...
Cris: Okay, but we first need a note in the Scripting API saying "we do not support profiles"

See also [comments taken on GitHub](https://github.com/w3c/wot-scripting-api/issues/531#issuecomment-3996664940)

#### [TD common definition and affects on formIndex  #582](https://github.com/w3c/wot-scripting-api/issues/582)

Ege: Criticism applies when using numbers for formIndex
...
Daniel: formIndex gives full power, which "prefer this or that contentType" does not provide
Ege: e.g., use case: prefer CoAP, and only if there are problems, HTTP
Cris: Nice experiments... but formIndex can already do all that
Ege: Helper functions
Cris: Correct, it could be made easy like this
...
Daniel: I still question the need... since formIndex can already do it
Cris: It could be part of "td-tools"
Daniel: Or clean up the TD before consuming it
Ege: I think developers should not need to see the expanded TD
Cris: Runtime will use the expanded TD
Ege: Is formIndex usable with the expanded TD?
Cris: Yes
