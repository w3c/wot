# WoT PlugFest Minutes 10.11.2025, 9am-6pm

## Attendees
Joshua Thomas (Ignite Retail Technology), Denis Ioan (ArmorSafe), Ege Korkan (Siemens), Tomoaki Mizushima (IRI), Kunihiko Toumura (Hitachi), Cu Pham (ECHONET), Mahda Noura (Siemens), Sebastian Kaebisch (Siemens), Piotr Sowinski (NeverBlink), Will Abramson (Legendary Requirements), Michael Robinson (ArmorSafe), Charith Perera (Cardiff University, Invited Expert), Kensaku Komatsu (NTT inc), Kaz Ashimura (Nagasaki University), David Ezell (Conexxus), Manu Sporny (Digital Bazaar), Brian McManus (Ignite Retail Technology)
    
Regrets: 

Chair: Sebastian

Scribe: Sebastian, all

## Logistics

Zoom Link
https://w3c.zoom.us/j/81402345014?pwd=uX2HqibWTyEBcerTXDzjvi9sdIBFBF.1

WIFIs
SSID: W3C-TPAC2025
Psw: kobe2025

Alternative:

SSID: NextSmartSystems
Psw: Loki44310448!


## Agenda and Links:

1. Welcome

2. Logistics (WIFI, Zoom link, room)

3. Round of introductions (What did you bring with you?; What you like to test / implement?)

4. Topic overview and shared TDs:

     https://github.com/w3c/wot-testing/tree/main/events/2025.11.Kobe

    please add your TDs here: https://github.com/w3c/wot-testing/tree/main/events/2025.11.Kobe/TD

     Hitachi: https://github.com/w3c/wot-testing/tree/main/events/2025.11.Kobe/TD/Hitachi

    OPC UA: https://github.com/w3c/wot-testing/blob/main/events/2025.11.Kobe/TD/OPC%20UA/

    ECHONET: https://github.com/w3c/wot-testing/tree/main/events/2025.11.Kobe/TD/ECHONET

    -> Web camera  to confirm the operation of devices: http://150.65.230.152/live/index.html

    username: [tan-lab] pwd: same as the username

    Coffee break: 10:30 to 11:00 

    Lunch 12:30-13:45

    Coffee break: 15:00 to 15:30


## Minutes
Mahda: focusing on the TD tooling, how to create ontologies. also focuisng on protocol bindings

Will: just observing WoT. Located in London. here to learn more

Piotr: CTO from Neverblink from Poland. try to integrate WoT Thing Description

Charith: Im from Sri Lanka and with Cardiff University. We have a smart home lab. Testing network behavoir and try to apply LLM. Try to find out the relevance of WoT for it. 

Josh: ArmorSafe is with Conexxus consortia. Using WoT Thing Description in the Conexxus standard and apply it to cashier system as example. We also invovled in other groups such in Web Payment WG

Phan: I'm with ECHONET. Standard from Japan. Combine ECHONET and Web of Things. I have different devices with me. Remotely there is also a smart home lab.

Kunihiko: I'm from Hitachi. I like to connect to other things via a MCP server approach.

Ege: Will testing some new TD 2.0 features. 

Sebastian: do some OPC UA testing. We want to publish the new companion standard soon

    github material: https://github.com/w3c/wot-testing/tree/main/events/2025.11.Kobe/TD/OPC%20UA


<Lunch break until 13:45>

 Kensaku: having spotlights connected to video content. If there specific metadata provided in the video spotlight can be moved and color can be changed. Communication protocol is called DMX. Metadata definition is similar defined as for subtitles in videos. 

     The presented slides can be found here:  https://nttcom.box.com/s/jm1c4o0v2qb3geppy73hboqjhd2w5tlu 

    DMX Synchronized Song "Angle Brackes Rise" video: https://nttcom.box.com/s/in7pki6cnjb1qiceh0ff6hgbhbuoevat

 Kaz: we can think about to have a DMX WoT Binding for it
 Pham: <shows the gitbub material>. Here are local devices. There is also a remote place which can be discovered with a web camera. Access will be shared (see above). Remote lab has over 20 devices, but only 11 devices are accessible this time. 

     Presented slides can be found here: https://github.com/w3c/wot-testing/blob/main/events/2025.11.Kobe/TD/ECHONET/2025TPAC.pdf

    github material https://github.com/w3c/wot-testing/tree/main/events/2025.11.Kobe/TD/ECHONET

 
 Related activity by Kensaku Komatsu, Sync on the Web CG
 
 * slide : https://nttcom.app.box.com/s/jm1c4o0v2qb3geppy73hboqjhd2w5tlu
 * demo movie : DMX Synchronized Song "Angle Brackes Rise" https://nttcom.box.com/s/in7pki6cnjb1qiceh0ff6hgbhbuoevat
 
 adjouned the meeting 18:12
